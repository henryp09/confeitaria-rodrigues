...
const AdminDashboard = () => {
  ...
  const stats = [
    ...
    { label: "Pedidos Hoje", value: "—", icon: ShoppingBag, color: "bg-accent" },
    { label: "Clientes", value: "—", icon: Users, color: "bg-blue-500" },
    { label: "Vendas Mês", value: "—", icon: TrendingUp, color: "bg-green-500" },
  ];
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {stats.map((stat) => (
            <div key={stat.label} className="bg-card rounded-xl p-6 shadow-sm border border-border/50">
                    ...
                    <tr key={product.id} className="hover:bg-secondary/30 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <img src={resolveImage(product.image_url)} alt={product.name} className="w-12 h-12 rounded-lg object-cover" />
                          <div>
                      ...
                      <td className="px-6 py-4">
                        <span className="px-3 py-1 bg-primary/10 text-primary rounded-full text-sm">{product.category}</span>
                      </td>
